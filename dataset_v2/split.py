...
def split_dataset(data_dir, train_val_test_dir, train_ratio, val_ratio, test_ratio):
    ...
    for file in train_files:
        ...
        src_label_path = os.path.join(data_dir, file.replace('.jpg', '.txt'))
        dst_image_path = os.path.join(train_dir, file)
        dst_label_path = os.path.join(train_dir, file.replace('.jpg', '.txt'))
    ...
    for file in val_files:
        ...
        dst_label_path = os.path.join(val_dir, file.replace('.jpg', '.txt'))
        shutil.copy(src_image_path, dst_image_path)
        shutil.copy(src_label_path, dst_label_path)
    ...
    for file in test_files:
        ...
        dst_label_path = os.path.join(test_dir, file.replace('.jpg', '.txt'))
        shutil.copy(src_image_path, dst_image_path)
        shutil.copy(src_label_path, dst_label_path)
...
# 设置数据集路径和切分比例
data_dir = 'E:/Py_code/yolov8/data/endDataCopy'  # 图片和标签路径
